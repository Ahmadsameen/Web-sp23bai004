...
const Complaint = require('./models/Complaint'); // Add this for the new model
...
const isAuthenticated = (req, res, next) => { // Add this middleware for logged-in users
    if (!req.session.user) {
...
app.post('/contact', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        res.redirect('/my-complaints?message=success');
    } catch (err) {
        console.log(err);
...
app.get('/my-complaints', isAuthenticated, async (req, res) => {
    const complaints = await Complaint.find({ userId: req.session.user.id }).populate('orderId');
...
app.get('/admin/complaints', isAdmin, async (req, res) => {
    ...
    res.render('admin/complaints', { title: 'Admin Complaints', complaints });
});
